    ...
    class Bdd
    {
        ...
        public Bdd(String UnLogin, String UnPwd)
        {
            ...
            try
            {
                ...
                ///<remarks>
                //On démarre avec une connexion depuis l'extérieur à la BdD
                // on va remplacer dans la chaine de connexion les paramètres 
            ...
            {
                // Si la conenxion depuis l'extérieur a échouée on tente de se connecté depuis la connexion
                try
                {
                    CnOracle = new OracleConnection(string.Format(CnString.ConnectionString,
        ...
        /// <summary>
        /// Surcharge de la fonction ObtenirDonnesOracle, permettant d'accepter la clause WHERE
        /// </summary>
        /// <param name="UneTableOuVue">nom de la table ou la vue dont on veut récupérer le contenu</param>
        /// <param name="Parametre">Parametre passé dans la clause where</param>
        /// <param name="NomParametre">Nom du Parametre passé dans la clause where (ex: where IdAtelier= ...)</param>
        /// <returns>un objet de type datatable contenant les données récupérées</returns>
        public DataTable ObtenirDonnesOracle(String UneTableOuVue, String Parametre, String NomParametre)
        {
            string Sql = "select * from " + UneTableOuVue + "  where "+ NomParametre + " = " +Parametre;
            this.UneOracleCommand = new OracleCommand(Sql, CnOracle);
            UnOracleDataAdapter = new OracleDataAdapter();
            UnOracleDataAdapter.SelectCommand = this.UneOracleCommand;
            UneDataTable = new DataTable();
        ...
        /// <summary>
        /// Méthode permettant de rajouter un atelier en base grace à la procédure "ins_atelier"
        /// </summary>
        /// <param name="pNomAtelier">Nom de l'atelier, libelle atelier en base</param>
        /// <param name="pNbPlace">Nombre de place maxi, nbplacesmaxi en base</param>
        public void CreerAtelier(String pNomAtelier, Int32 pNbPlace)
        {
            try
            {
                UneOracleCommand = new OracleCommand("ins_atelier", CnOracle);
                UneOracleCommand.CommandType = CommandType.StoredProcedure;
                UneOracleCommand.Parameters.Add("pLibelleAtelier", OracleDbType.Varchar2, ParameterDirection.Input).Value = pNomAtelier;
                UneOracleCommand.Parameters.Add("pNbPlacesMaxi", OracleDbType.Int32, ParameterDirection.Input).Value = pNbPlace;
                UneOracleCommand.ExecuteNonQuery();
                MessageBox.Show("Atelier créé !");
            }
            catch (OracleException Oex)
            {
                MessageBox.Show("Erreur Oracle \n" + Oex.Message);

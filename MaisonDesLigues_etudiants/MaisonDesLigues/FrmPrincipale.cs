...
namespace MaisonDesLigues
{
    public partial class FrmPrincipale : Form
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (this.cmbElement.SelectedItem.ToString())
            {
                case "Atelier":
                    GererCreationAtelier();
                    break;
                case "Theme":
                    GererCreationTheme();
                    break;
                case "Vacation":
                    GererCreationVacation();
                    break;
        ...
        private void GererCreationAtelier()
        {

            grpTheme.Visible = false;
            grpAtelier.Visible = true;
            grpVacation.Visible = false;
            
        }

        private void GererCreationTheme()
        {

            grpTheme.Visible = true;
            grpAtelier.Visible = false;
            grpVacation.Visible = false;
            grpTheme.BringToFront();
            grpTheme.Invalidate();
            //Utilitaire.CreerDesControles(this, UneConnexion, "VSTATUT01", "Rad_", PanFonctionIntervenant, "RadioButton", this.rdbStatutIntervenant_StateChanged);
            Utilitaire.RemplirComboBox(UneConnexion, cmbThemeAtelier, "VATELIER01");

            cmbThemeAtelier.Text = "Choisir";

        }

        private void GererCreationVacation()
        {

            grpTheme.Visible = false;
            grpAtelier.Visible = false;
            grpVacation.Visible = true;
            grpVacation.BringToFront();
            //Utilitaire.CreerDesControles(this, UneConnexion, "VSTATUT01", "Rad_", PanFonctionIntervenant, "RadioButton", this.rdbStatutIntervenant_StateChanged);
            Utilitaire.RemplirComboBox(UneConnexion, cmbVacationAtelier, "VATELIER01");
            cmbVacationAtelier.Text = "Choisir";
            cmbVacationTheme.Enabled = false;
        ...
        private void cmbVacationAtelier_SelectedIndexChanged(object sender, EventArgs e)
        {
            cmbVacationTheme.Enabled = true;
            //(cmbVacationTheme.Items.Clear();
            cmbVacationTheme.SelectedIndex = -1;
            if (cmbVacationAtelier.SelectedValue.ToString() != "System.Data.DataRowView")
            {
                Utilitaire.RemplirComboBox(UneConnexion, cmbVacationTheme, "VTHEME01", cmbVacationAtelier.SelectedValue.ToString(), "idatelier");
            }
            if (cmbVacationTheme.Items.Count == 0)
            {
                cmbVacationTheme.Text = "Aucun thème disponible";
                cmbVacationTheme.Enabled = false;
            }
        }
